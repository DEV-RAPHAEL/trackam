...
export async function GET(req: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  try {
    await initDb();
    const result = await db.query('SELECT * FROM clients WHERE company_id = $1 ORDER BY created_at DESC', [companyId]);
    return NextResponse.json(result.rows);
  } catch (e: any) {
    console.error(`API Error [clients/company/${companyId}]:`, e);
